...
export const leadAdapter: TargetAdapter = {
  ...
  async createFrom(db, { orgId, values, branchId, academicYearId }) {
    ...
    const dedup = await findMatches(db, {
      ...
      childName: base.kidName as string | undefined,
      grade: base.gradeSought as string | undefined,
      academicYearId: academicYearId ?? null,
    }, dedupConfig)
    const hardLeadMatch = dedup.hardBlock ? dedup.matches.find(m => m.type === 'lead' && m.action === 'hard') : undefined
    if (hardLeadMatch) {
      await db.lead.update({ where: { id: hardLeadMatch.id }, data: base })
      return { id: hardLeadMatch.id }
    }
