...
/**
 * sendEmail
 * Helper function to dispatch emails via Resend. Falls back to console logs in development if key is empty.
 */
export async function sendEmail(
  ...
  const fromEmail = from || process.env.RESEND_FROM_EMAIL || 'Vidhyaan <no-reply@vidhyaan.com>'
  const isDev = process.env.NODE_ENV === 'development'
  const apiKey = process.env.RESEND_API_KEY

  if (!apiKey || isDev) {
    console.log('='.repeat(60))
    console.log('[Resend Mock] Email notification triggered:')
  ...
  try {
    ...
    if (data.error) {
      ...
      throw new Error(data.error.message || 'Resend delivery failed')
    }

    return data
  } catch (err: any) {
    console.error('[Resend] Error executing send:', err)
    throw err
  }
}
...
/**
 * sendTemplateEmail
 * Helper combining template rendering and dispatch
 */
